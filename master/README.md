...
### 時間外の計算（会社の数字と突き合わせて確定させた）

```
付いた分        = 労働時間 と 拘束時間×0.8 の多いほう
走らなかった分  = 拘束時間 × 0.8
時間外          = 付いた分 − 走らなかった分
```

公休出勤の日は「走らなかった分」が無いので、付いた分がそのまま時間外になる。
そのまま乗った分は自分の勤務なので、引き算には出てこない。

確定までの経緯（推測で決めず、実例で1つずつ確かめた）:

| 日 | 内容 | アプリ | 会社 |
|---|---|---|---|
| 2026-09-08 | 公休出勤 38番 | 8:22 → **9:39** | 9:39 |
| 2026-09-09 | 73番の後半を6番の後半へ振替 | 0:37 → **0:51** | 0:51 |

- 9/8 で「拘束×0.8 だけ」では足りず、労働時間と比べて多いほうを使うと判明
- 9/9 で「引く側は労働ではなく拘束×0.8」と判明。月合計のズレ約5時間はこれが原因
- **数字が合わないときは、推測で式を変えず、実例を1件もらって逆算すること**
